...
use crate::{
    ...
    SparkMakeTimestamp, SparkNextDay, SparkSecondsToTimestamp, SparkSizeFunc,
};
...
pub fn create_comet_physical_fun_with_eval_mode(
    ...
    match fun_name {
        ...
        // make_date, make_timestamp, and next_day must be constructed with the ANSI flag
        // (fail_on_error) so they throw on invalid input under ANSI rather than returning NULL.
        "make_date" => Ok(Arc::new(ScalarUDF::new_from_impl(SparkMakeDate::new(
            fail_on_error,
        )))),
        "make_timestamp" => Ok(Arc::new(ScalarUDF::new_from_impl(
            SparkMakeTimestamp::new(fail_on_error),
        ))),
...
fn all_scalar_functions() -> Vec<Arc<ScalarUDF>> {
    vec![
        ...
        Arc::new(ScalarUDF::new_from_impl(SparkMakeTimestamp::default())),
